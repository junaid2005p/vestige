...
func (r *Repository) DeleteSnapshot(id string, dryRun bool, staleAfter time.Duration) (DeleteStats, error) {
	...
	defer lock.release()
	if _, err := r.recoverStaging(); err != nil { return stats, err }
	...
	for _, entry := range manifest.Files {
		...
		for _, ref := range entry.Chunks { seen[ref.ID] = struct{}{} }
	}
	stats.ReferencedChunks = len(seen)
	if dryRun { return stats, nil }
	source := filepath.Join(r.snapshotsDir(), id)
	trash := filepath.Join(r.tmpDir(), fmt.Sprintf("deleted-snapshot-%s-%d", id, time.Now().UnixNano()))
	if err := os.Rename(source, trash); err != nil { return stats, fmt.Errorf("unpublish snapshot %s: %w", id, err) }
	if err := os.RemoveAll(trash); err != nil { return stats, fmt.Errorf("remove unpublished snapshot %s: %w", id, err) }
	return stats, nil
}
